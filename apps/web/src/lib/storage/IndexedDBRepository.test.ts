...
describe('IndexedDBRepository', () => {
  ...
  it('should throw error when trying to delete the last profile', async () => {
    ...
    await expect(repo.deleteProfile(profile.id)).rejects.toThrow('Cannot delete the last remaining profile');
  });
  ...
  it('should throw error when trying to rename to existing name', async () => {
    ...
    await expect(repo.renameProfile(profile1.id, 'name2')).rejects.toThrow('A profile with the name "name2" already exists');
  });
  ...
  it('should handle multiple resource types for the same profile', async () => {
    ...
    await repo.addEntry('bug_rtb', profile.id, { id: '3', timestamp: '2024-01-03T00:00:00Z', rareDrops: 0 });
